...
get_myPoints.myDSD_Gaussians <- function(x, n=1, 
  ...
  data <- t(as.data.frame(mclapply(clusterOrder, FUN = function(i) MASS::mvrnorm(1, mu=x$mu[i,], Sigma=x$sigma[[i]]), mc.cores = 8)))			
  rownames(data) <- NULL
...
stream <- myDSD_Gaussians(k = 10, d = d)

dataPoints <- get_myPoints.myDSD_Gaussians(stream, n = 20000, cluster = TRUE)
dataPoints$Class <- attr(dataPoints, 'cluster')
write.csv(dataPoints, file = 'dataset.csv', row.names = FALSE)
