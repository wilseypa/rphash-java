...
data <- read.csv("C:/Users/user/Desktop/cnae9_rphash_intconf/cnae9.csv", header = FALSE)
...
files <- list.files(path = "C:/Users/user/Desktop/cnae9_rphash_intconf/", pattern = "*.csv", full.names = TRUE)
lapply(files, function(i) {
  ...
  if (ncol(f) == n) {
    for (j in 1:n) {
      ...
      if (length(unique(f[,j])) != 1) {
        ...
        sink(file = "C:/Users/user/Desktop/cnae9_rphash_intconf/Message.txt")
    ...
    row1 <- paste0(i, ",", ari_mean, ",", ari_sd, ",", purity_mean, ",", purity_sd, ",", silWidth_mean, ",", silWidth_sd,
                     ",", wcsse_mean, ",", wcsse_sd)
    write.table(row1, file = 'C:/Users/user/Desktop/cnae9_rphash_intconf/TestMeasures.csv', append = TRUE, quote = FALSE, sep = ",",
  ...
  if (ncol(f) == 18) {
    ...
    runtimes_sd <- sd(runtimes)
    memory_mean <- mean(memory)
    memory_sd <- sd(memory)

    row2 <- paste0(i, ",", runtimes_mean, ",",runtimes_sd, "," ,memory_mean ,"," , memory_sd )
    write.table(row2, file = 'C:/Users/user/Desktop/cnae9_rphash_intconf/TimeMemory.csv', append = TRUE, quote = FALSE, sep = ",",
                row.names = FALSE, col.names = FALSE)
  }
})
