...
public class RPHashMultiProj  implements Clusterer{
	...
	public RPHashObject mapP1(RPHashObject so) {
		...
		HashAlgorithm hal = new NoHash(Long.MAX_VALUE);// MurmurHash(so.getHashmod());
	...
	private  void run(RPHashObject so)
	{
		...
		List<float[]> centList = new RPHashSimple(so).getCentroids();
		
		for(int i = 1;i<so.getProbes();i++)centList.addAll(new RPHashSimple(so).getCentroids());
		
		centroids = centList; //( new Agglomerative(so.getk(),so.getCentroids())).getCentroids();
		//centroids = centroids;
	}
	...
	public static void main(String[] args) {
		...
		int d = 5000;
		int n = 10000;
		GenerateData gen = new GenerateData(k, n / k, d,1.1f,true);
