...
public class DBFriendlyProjection implements Projector {
	...
	int[][] GenRandom(){
	 ...
	 // float scale = (float)Math.sqrt(3.0f/(m));
	  ...
	  for(int i=0;i<t;i++)
	  {
		  ...
		  {
	        r = rand.nextInt(6);
	        if(r==0)ordered.add(j);
		  }
		  tmp = new int[ordered.size()];
		  int j = 0;
		  for(Integer in: ordered)tmp[j++] = in.intValue();
	...
	//v: the input vector
	//P: the size [t x n/6]  set of vector indices that should be positive +sqrt(3/t)
	//M: the size [t x n/6]  set of vector indices that should incur negative -sqrt(3/t)
