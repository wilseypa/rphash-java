...
/**
 * Work In Progress to implement the Fast Johnson Lindenstrauss transform under
...
public class FJLTProjection implements Projector {
	...
	public FJLTProjection(int n, int t, int numvec) {
		r = new Random();
		this.num_vectors = numvec;
		this.t = t;
		this.n = n;

		float eps = (float) Math.sqrt(Math.log(n) / t);

		D = generated(n);
		P = generatep(numvec, t, n, eps, 2);
	}

	public FJLTProjection(int s) {
		this.num_vectors = s;
	}

	float[] cblas_sgemv(int t, int n, float alpha, float[] M, float[] v,
			int startpoint, float[] result, int startoutput) {
		int i, j;
		float sum;
		for (i = 0; i < t; i++) {
			sum = 0.0f;
	 ...
	 * Generates P matrix
	 * 
	 * INPUT: size of distribution (k,d) epsilon e embedding type p number of
	 * points n OUTPUT: P matrix
	 * 
	 * Algorithm: P_ij = N(0,1/q) with probability q = 0 elsewhere
	 * 
	 * q = min( ( e^p-2 * (log n)^p /d ), 1 ) p belongs to {1,2}
	 */
	float[] generatep(int n, int k, int d, float e, int p) {
		...
		float[] data = inv_randn( k, d, 0f, 1f / (float) q);
		float[] rdata = randu( k, d);
	 ...
	 * Normal Distribution
	 * 
	 * INPUT: Pointer to hold the distribution data Size of Distribution (m,n)
	 * mean mu, variance var OUTPUT: Matrix filled with normal distribution
	 * 
	 * Algorithm: Uses Moro's Inverse CND distribution to generate an arbitrary
	 * normal distribution with mean mu and variance var
	 */
	float[] inv_randn(int m, int n, float mu, float var) {
	 ...
	 */
	float[] randu( int m, int n) {
		float[] data = new float[t * n];
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
	 ...
	 * 
	 * INPUT: size of matrix d OUTPUT: Matrix D
	 * 
	 * Algorithm: D_ii = {-1,1} with probability 0.5
	 * 
	 * Returned as a vector with length d to avoid extra space seems to be
	 * correct
	 */
	...
	float[] FJLT(float[] input) {
		...
		float[] result = new float[num_vectors * t];
		 ...
		 */
		int curr = 0;
		for (curr = 0; curr < num_vectors; curr++) {
			int startpoint = curr * n;
			// float * point = data[startpoint];
			int startoutput = t * curr;
			 ...
			 */
			int l2 = (int) (Math.log(n) / Math.log(2));
			for (a = 0; a < l2; a++) {
				for (b = 0; b < (1 << l2); b += (1 << (a + 1))) {
					for (c = 0; c < (1 << a); c++) {
						float temp = input[startpoint + b + c];
						input[startpoint + b + c] += input[startpoint + b + c
								+ (1 << a)];
						input[startpoint + b + c + (1 << a)] = temp
								- input[startpoint + b + c + (1 << a)];
					}
				}
			}
			 ...
			 * Multiply with P
			 */
			result = cblas_sgemv(t, n, 1.0f / (float) n, P, input, startpoint,result,
					startoutput);
		}
		return result;
	}
	...
	public float[] project(float[] input) {
		...
		result = cblas_sgemv(t, n, 1.0f / (float) n, P, input, 0, result, 0);
		return result;
	}
	...
	public static void testALL() {
		...
		float[] data = { 1.f, 0.f, 2.f, 7.f, 4.f, 0.f, 8.f, 3.f, 2.f, 1.f, 4.f,
				0.f, 8.f, 3.f, 2.f, 1.f, 4.f, 2.f, 4.f, 5.f, 3.f, 2.f, 1.f,
				1.f, 4.f, 2.f, 4.f, 5.f, 1.f, 0.f, 2.f, 7.f, 4.f, 0.f, 8.f,
				4.f, 0.f, 8.f, 3.f, 2.f, 0.f, 8.f, 3.f, 0.f, 8.f, 3.f, 2.f,
				1.f, 4.f, 2.f, 4.f, 5.f, 2.f, 1.f, 4.f, 1.f, 0.f, 2.f, 7.f,
				4.f, 1.f, 3.f, 2.f, 1.f, 4.f, 0.f, 8.f, 3.f, 2.f, 1.f, 4.f,
				2.f, 4.f, 5.f, 0.f, 2.f, 7.f, 4.f, 0.f, 8.f, 1.f, 0.f, 2.f,
				7.f, 0.f, 8.f, 3.f, 2.f, 1.f, 4.f, 2.f, 4.f, 4.f, 0.f, 8.f,
				3.f, 2.f, 1.f, 4.f, 5.f, 4.f, 0.f, 8.f, 3.f, 2.f, 1.f, 4.f,
				0.f, 8.f, 3.f, 2.f, 1.f, 4.f, 2.f, 4.f, 5.f, 1.f, 0.f, 2.f, 7.f };
	...
	public void init() {
		r = new Random();
		float eps = (float) Math.sqrt(Math.log(num_vectors) / t);
		D = generated(this.n);
		P = generatep(this.num_vectors, this.t, this.n, eps, 2);
	}
	...
	// int d = 10;
	// float[] data =
	// {1.f,0.f,2.f,7.f,4.f,0.f,8.f,3.f,2.f,1.f,4.f,0.f,8.f,3.f,2.f,1.f,4.f,2.f,4.f,5.f};
	//
