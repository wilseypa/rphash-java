...
public class ProjectionTests {
	static void testRPH(int n,int d,int multi){
		...
		for(int i =0;i<gen.data.size();i+=2){
			...
			float[] f1 = VectorUtil.avgProjection(gen.data.get(i), p1, 24);
			float[] f2 = VectorUtil.avgProjection(gen.data.get(i+1), p2, 24);
			sum+=VectorUtil.distance(f1,f2);
			sumreal +=VectorUtil.distance(gen.data.get(i),gen.data.get(i+1));

			//get a random vector
			float[] r2 = gen.data.get(rand.nextInt(gen.data.size()));
			sumunreal +=VectorUtil.distance(gen.data.get(i),r2);
			f2 = VectorUtil.avgProjection(r2, p1, 24);
			sumun+=VectorUtil.distance(f1,f2);
